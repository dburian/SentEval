...
class STSEval(object):
    def loadFile(self, fpath):
        ...
        for dataset in self.datasets:
            ...
            # Ugly fix from https://github.com/facebookresearch/SentEval/issues/94
            ...
            sent1 = np.array(
                [s.split() + [""] * (max_length - len(s.split())) for s in sent1]
            )[not_empty_idx]
            sent2 = np.array(
                [s.split() + [""] * (max_length - len(s.split())) for s in sent2]
            )[not_empty_idx]
            # Original code:
